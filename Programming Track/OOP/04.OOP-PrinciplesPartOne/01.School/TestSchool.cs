using System;
using System.Collections.Generic;
using System.Linq;

//01.We are given a school. In the school there are classes of students. 
//Each class has a set of teachers. Each teacher teaches a set of disciplines. 
//Students have name and unique class number. Classes have unique text identifier. Teachers have name.
//Disciplines have name, number of lectures and number of exercises. Both teachers and students are people. 
//Students, classes, teachers and disciplines could have optional comments (free text block).
//Your task is to identify the classes (in terms of  OOP) and their attributes and operations, encapsulate their fields, 
//define the class hierarchy and create a class diagram with Visual Studio.

namespace _01.School
{
    public class TestSchool
    {
        static void Main(string[] args)
        {
            List<Student> students = new List<Student>();
            students.Add(new Student("Ivan", 15, 12345));
            students.Add(new Student("Peter", 15, 12346));
            students.Add(new Student("Georgi", 16, 12347));

            List<Discipline> listOfTeahcersDiscipline=new List<Discipline>();
            listOfTeahcersDiscipline.Add(new Discipline("Literature",10,15));
            listOfTeahcersDiscipline.Add(new Discipline("Mathematics",10,15));

            Teacher teacherOfTheClass=new Teacher("Anna",43,listOfTeahcersDiscipline);

            ClassOfStudents classOfStudents = new ClassOfStudents(students, teacherOfTheClass, "9B");
            classOfStudents.PrintClass();
        }
    }
}
